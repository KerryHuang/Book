---
kind: reprint
source: https://igouist.github.io/post/2021/04/newbie-1-hello-git
author: igouist
---
...
但由於我個人忘記指令的狀況頗為嚴重，而且也不是每個環境都有 GUI 可以用。因此這篇有關 Git 的部分將會以 CLI 指令為主進行紀錄。使用 GUI 介面的朋友也不用擔心，現在的介面都做得很精簡，而且這篇也寫得很淺，很簡單就能找到各指令對應的操作。
...
這樣就會把設定好的名稱和信箱存到 config 裡面。如果要查看現在有的 config，可以輸入
`git config --list`
...
> 順便打個廣告：好奇我的 Powershell 長得跟你的「有點不一樣」的朋友，可以參考本部落格的另一篇：[Powershell 美化作戰](https://igouist.github.io/post/2020/08/powershell-beauty/)

## 建立一個新的儲存庫（Init）

現在讓我們從建立一個新的儲存庫開始。現在讓我們新增一個資料夾（在這邊我取名叫做 `hello-git`）當作這篇 Git 紀錄的遊樂場：

![img](https://i.imgur.com/hPLoleB.png)

> 題外話：既然都打指令了，也可以試試來建立資料夾
> Powershell 用 `New-Item C:\hello-git -ItemType "directory"`
> 隔壁棚 Linux 請用 `mkdir` 來試試。
> 不過基於懶惰，上面的示範是滑鼠右鍵建立的，耶嘿
...
並且使用 `git init` 將 Git 初始化：

```powershell
git init
```

![img](https://i.imgur.com/WIckbA1.png)

如此一來，這個資料夾就成為了「工作區（Working directory）」，也就是「歸我 Git 管啦！」的意思

同時，我們也能在原本的 hello-git 資料夾中，發現多了一個 `.git` 的隱藏檔案：

![img](https://i.imgur.com/3hgT4eP.png)

這個 `.git` 就是用來幫我們處理一堆版本控制工作的地方，也叫做
「儲存庫（Repository）」。裡面會放一些設定值、已經確認變更的文件等等。

現在讓我們來確認一下這個工作區的狀況，`git status` 將會列出當前的狀態，這將會是我們很常使用的指令。

![img](https://i.imgur.com/5XwsruS.png)
...
![img](https://i.imgur.com/X8M9I67.png)

並且加入一些內容，例如「Hello!」

![img](https://i.imgur.com/erpTd0w.png)

現在讓我們再次使用 `git status` 觀察一下

![img](https://i.imgur.com/J7ByoFh.png)

可以看到 Git 已經發現 A.txt 的存在了，但它也告訴我們，它還沒有把 A.txt 放在 眼裡 追蹤目標中
...
![img](https://i.imgur.com/fFlY2rK.png)
...
![img](https://i.imgur.com/dARJCu3.png)
...
![img](https://i.imgur.com/riSISaf.png)
...
這邊要特別提的是 `-m "Add A.txt"` 這個部分。`-m` 就是 Message 的 m（好順口），是用來輸入本次 Commit 的訊息，雖然可以省略，但強烈建議 Commit 的時候都一定要加上訊息！
...
> 關於 Commit Message 的部分，可以參照這幾篇呦：
> 。 [Git Commit Message 這樣寫會更好，替專案引入規範與範例](https://wadehuanglearning.blogspot.com/2019/05/commit-commit-commit-why-what-commit.html)
> 。 [如何寫一個 Git Commit Message | louie_lu’s blog](https://blog.louie.lu/2017/03/21/如何寫一個-git-commit-message/)
> 。 [撰寫有效的 Git Commit Message](http://blog.fourdesire.com/2018/07/03/撰寫有效的-git-commit-message/)
> 。 [[學習筆記\] 如何撰好的 Git Commit Message - Heidi’s Blog](https://heidiliu2020.github.io/git-commit-message/)
...
寧可多 Commit 幾次，等熟練 Git 的時候，或是被靠夭洗版的時候，再考慮用 rebase 之類的技能來把多個零碎的 Commit 整理成一個；也不要臨時出了什麼事，結果 Git 一打開，只能回到一個月前，那真的是欲哭無淚。阿彌陀佛，保護自己，就從 Commit 開始。

> 如果你現在已經有整理 Commit 的需求，可以參照以下幾篇：
> 。 [【狀況題】修改 Commit 紀錄 - 為你自己學 Git](https://gitbook.tw/chapters/using-git/amend-commit1.html)
...
![img](https://i.imgur.com/4QWKh8Y.png)
...
![img](https://i.imgur.com/k0sNgcN.png)
...
現在我們已經可以成功存檔了，但有玩過遊戲的都知道，通常都會有個地方讓你看當前的存檔和紀錄等等。那麼在 Git 中要怎麼看我們的 Commit 記錄呢？這時候就要用到 `log` 指令。

由於只有存過一次也太寒酸了，現在讓我們把 A.txt 打開，將原本的 Hello! 改成 Hello world! 並且再 Commit 一次：

![img](https://i.imgur.com/uZY0Kjm.png)

![img](https://i.imgur.com/IhLiS0z.png)

然後，讓我們試試 `git log`：

![img](https://i.imgur.com/hD1c1h1.png)
...
- 分支所在的 Commit
  ...
  - 關於 HEAD 我們會在 [關於 HEAD](https://igouist.github.io/post/2021/04/newbie-1-hello-git/#關於-head) 繼續說明
  - 關於 分支 我們會在 [分支](https://igouist.github.io/post/2021/04/newbie-1-hello-git/#分支branch) 繼續說明

> 題外話：如果有朋友在 windows 上，例如 powershell 使用 `git log` 或 git 相關指令有出現中文亂碼的情形，可以參考以下的語法進行調整：
>
> ```
...
不過比起這樣的文字，我們比較常用的還是 Git 那相當有特色的線圖。現在讓我們試試
`git log --oneline --graph`：

![img](https://i.imgur.com/V9ysh2L.png)

可以看到 Log 變得相當精簡了。在我們後續會講到的[分支](https://igouist.github.io/post/2021/04/newbie-1-hello-git/#分支branch)情景下，線圖還會幫忙畫出不同分支路線（平常有使用 Git 的 GUI 工具的朋友們應該都很熟悉了）

這邊就借朋友的 Project 示範一下：

![img](https://i.imgur.com/lHYIpqd.png)
...
![img](https://i.imgur.com/9BYAkeV.png)
...
這時候我們就可以使用 `git diff` 來看兩個 Commit 之間的差異。
...
![img](https://i.imgur.com/HLjl7aj.png)
...
![img](https://i.imgur.com/8WlJXkE.png)

接著讓我們用 `git status` 確認一下：

![img](https://i.imgur.com/zOf9MDZ.png)

可以看到工作區已經有變更了，但我們尚未 `add` 到暫存區裡。

現在讓我們使用 `git diff` 指令來看工作區和儲存庫間的差異：

![img](https://i.imgur.com/T2ajGae.png)
...
從上面的許多操作中，例如 [顯示 Commit Log 時](https://igouist.github.io/post/2021/04/newbie-1-hello-git/#歷史紀錄log)，都可以看到 HEAD 這個關鍵字，例如 (HEAD -> master)。

HEAD 基本上可以當作「目前位置」的概念，它是一個會指向當前分支的指標。

通常來說，HEAD 會指向目前所在的分支最新的一個 Commit（除非發生[斷頭](https://igouist.github.io/post/2021/04/newbie-1-hello-git/#關於斷頭detached-head)）。
...
![img](https://i.imgur.com/WWGzQQh.png)
...
![img](https://i.imgur.com/P7kJY5C.png)

接著再確認一次 Log：

![img](https://i.imgur.com/lqLNNYX.png)

可以看到又多了一個 Commit，這個 Commit 用來撤回前一個 Commit。

![img](https://i.imgur.com/PhPCFqm.png)
...
如果突然又不想撤回了怎麼辦？你可以再 Revert 一次來 Revert 掉上個 Revert 藉此還原上個 Revert 所 Revert 掉的 Commit，夠直覺吧！
...
![img](https://i.imgur.com/ltpNAm7.png)
...
![img](https://i.imgur.com/TdP0vZf.png)

Git 跟我們說「你已經回到了 `6fbe` 這個 Commit 囉」接著讓我們確認看看 A.txt：

![img](https://i.imgur.com/ijZA2K2.png)
...
分支在大多範例中會被形容像是樹枝的分枝，從主幹發散出去。但我個人覺得比較像是水流，從任何一處我們都可以岔分出另一條水流，這兩條水流各自經歷了不同的地方，可能第一條水流帶來了泥沙，第二條河流被汙染了核廢料，而我們又能將這兩條水流會合在一起，得到同時有泥沙和核廢料的水流。

而個人認為，分支最重要的概念就是進行環境的隔離，大家可以在自己的分支上進行作業，又不會互相影響。就算想修改看看，也可以在自己的分支上先行測試，沒問題了再合併上去，這樣就能在工作流程上貫徹封裝、降耦合和單一職責的精神。並且，也因為多條分支可以同時進行不同的工作，也能達到平行處理般的效率，重要的是戰犯更好找了，豈不妙哉。
...
![img](https://i.imgur.com/gEWbVkJ.png)

接著將這兩個新檔案 Add 進來並且 Commit：

![img](https://i.imgur.com/qXwq3WC.png)

可以看到我們的 master 分支位於最新的 Commit 07672f4 上了，接著我們要從這裡
用 `git branch` 拉出一條新的分支：

```
git branch Branch-B
```

> 補充：分支拉出來的 Commit 點稱為 `Base`。由於任兩個 Branch 一定會找得到一個共同的 base，因此我們在對分支間做合併和比較時，並不是將整份拿出來做大量核對，而是從這個 Base 當作基準點來進行變更的比較。這個 base 的概念和我們後續的 [合併](https://igouist.github.io/post/2021/04/newbie-1-hello-git/#合併merge)、[衝突](https://igouist.github.io/post/2021/04/newbie-1-hello-git/#衝突conflict) 等操作會比較相關。

> 補充：如果建立分支的時候不小心取錯名字之類的，可以先偷看後面的 [查詢分支列表、刪除分支](https://igouist.github.io/post/2021/04/newbie-1-hello-git/#查看分支列表刪除分支) 的小節。如果一切順利的話，我們後續在 ~~利用完分支~~ 跑完整個流程才會進行分支的刪除，請繼續看下去囉～

建立分支後，我們再使用 `git checkout` 簽出分支：
...
![img](https://i.imgur.com/mrektkG.png)
...
![img](https://i.imgur.com/2cxGS7c.png)
...
![img](https://i.imgur.com/o68UUxJ.png)

接著移除 C.txt 之後 Commit：

![img](https://i.imgur.com/r5eNNrE.png)
...
> 「真是太諷刺了紹安，你拉了新分支繞了一大圈，
> 　最後做出來的 feature 竟然是你不想做的，你老闆的需求。
> 　所以說呢，分支最後終究是要回到 master 來的，
> 　這四千個 Commit 的盡頭 Merge，或許正是你的極限也說不定。」
...
接著使用 `git merge` 指令來用 Branch-B 分支把 Branch-C 分支給吃進來：

![img](https://i.imgur.com/9G1qj6g.png)

這時候可以觀察看看 `hello-git` 資料夾，B.txt 和 C.txt 應該都已經消失了。

我們可以用 `git log` 來觀察一下：

![img](https://i.imgur.com/XHzq9yk.png)
...
所以合併的時候要注意一下當前的分支，是將目標分支匯入到當前所在的分支，不要弄反了，不然把一堆測試用的東西合併上去正式環境就完蛋啦。

現在我們再把 Branch-B 的變更也合併進 master 吧。

同樣地，先簽出 master，再 `git merge Branch-B`

![img](https://i.imgur.com/pEySmri.png)
...
當然，不是每一次合併都能順利，就像不是每次合作都能順利一樣。只要兩個分支有共同增刪改同一份文件同一個區塊，在合併的時候就會發生衝突。
...
![img](https://i.imgur.com/WeFQiSM.png)
...
![img](https://i.imgur.com/us5hqzk.png)
...
![img](https://i.imgur.com/4j9GJmA.png)

可以看到 Git 跳出訊息跟你說自動合併失敗（Automatic merge failed），要求你進行修復。

並且 `git status` 也會告訴你，你有未處理的 merge 問題，A.txt 這個檔案都有被變更到：

![img](https://i.imgur.com/14o8dB9.png)

這時候讓我們去看看 A.txt 的內容：

![img](https://i.imgur.com/etAf0gj.png)

會看到 Git 標示出發生衝突的部分，上半部是我們當前 HEAD 也就是 Branch-Y 的內容，下部分則是 Branch-X 的內容。

我們必須把這個衝突的部分處理好，並且把這些標示拿掉，再重新 Commit 一次。假設Ｘ跟Ｙ兩位同事經過 ~~扭打~~ 討論之後，決定改成兩個人的名字都要標在上面，變成 `Hello X & Y!`，我們就可以進行 commit 來結束衝突囉：

![img](https://i.imgur.com/1HXkePJ.png)

通常來說，如果是使用 GUI 工具的朋友，應該會像 `Diff` 那樣給一個對照的列表讓你選擇，例如說我平常用的 Visual Studio 就會並排讓你勾選要使用哪一邊的，或是自己修改（這邊借用一下官方文件的圖）：

![img](https://docs.microsoft.com/zh-tw/visualstudio/ide/media/git-merge-editor.png?view=vs-2019)
...
![img](https://i.imgur.com/sBPfJRR.png)
...
![img](https://i.imgur.com/QPcfALx.png)
...
![img](https://i.imgur.com/CaYIfDt.png)

接著再讓我們用 `git branch` 確認一下吧：

![img](https://i.imgur.com/2aslDOI.png)
...
- [團隊的 GIT 分支管理策略 (1) ： 基本概念](https://medium.com/哈嘍-世界/團隊的-git-分支管理策略-449bc229c957)
- [團隊的 GIT 分支管理策略 (2) ： 主線整合與功能分支](https://medium.com/哈嘍-世界/團隊的-git-分支管理策略-2-整合頻率對團隊效率的影響-bedbbdd3e70e)
...
![img](https://i.imgur.com/Nci6Yvd.png)

接著讓我們用 `git remote add` 來增加叫做 `origin` 的遠端儲存庫到這個儲存庫中，完成了之後再用 `git remote` 確認遠端儲存庫列表是不是已經有 `origin` 了：
...
![img](https://i.imgur.com/WFqrEAu.png)
...
![img](https://i.imgur.com/vNFIUiQ.png)
...
![img](https://i.imgur.com/Ubdt85H.png)

推送完成後，讓我們再回到 Github 上查看：

![img](https://i.imgur.com/SNLyXDy.png)

可以看見東西都已經推送上去了。這時候按下旁邊的 Commit，也能夠查看這個儲存庫的 Commit Log 囉：

![img](https://i.imgur.com/DY9AStm.png)
...
![img](https://i.imgur.com/cVtjDCx.png)

然後按下編輯：

![img](https://i.imgur.com/9SU7PR1.png)

把內文稍微修改一下，例如改成 `Hello X & Y & Z!`

![img](https://i.imgur.com/TRv8i93.png)

拉到最下面，加上 Commit Message，並且按下 Commit Changes

![img](https://i.imgur.com/GRpH0el.png)

現在遠端儲存庫已經有了一個變更，但是我們本機還不知道呢！接著就讓我們實際操作一下，首先我們需要取得遠端的資訊，這時候就可以用 `fetch` 來擷取遠端儲存庫的資訊回來：

```
git fetch
```

![img](https://i.imgur.com/InviqZr.png)

可以看見 Git 從遠端儲存庫抓了資訊回來，這時候我們如果查看 `git log`，並加上 `--all` 來顯示全部分支的話：

![img](https://i.imgur.com/JBEalid.png)
...
當然我們也可以不用這麼麻煩，可以直接使用 `pull` 指令來提取遠端儲存庫對應的分支直接和本機現在的分支進行合併，也就是 `fetch` + `merge`。這樣就是從雲端下載最新檔囉！現在就讓我們試試吧：
...
![img](https://i.imgur.com/945NY8u.png)
...
![img](https://i.imgur.com/TmPSidi.png)

接著在 `GitRepos` 裡面就生出來一個 `hello-git` 啦，進入後檔案也都在呢：

![img](https://i.imgur.com/qJ9RVQN.png)
...
> 本系列下一篇：[菜雞新訓記 (2): 認識 Api & 使用 .net Core 來建立簡單的 Web Api 服務吧](https://igouist.github.io/post/2021/05/newbie-2-webapi)
