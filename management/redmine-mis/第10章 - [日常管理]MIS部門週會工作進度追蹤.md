...
本次管理需求限MIS內部成員使用，所以我們將本專案設定成[不公開]，只有專案成員才有權限看到專案資訊。
...
被指定的同仁進來編輯此筆Ticket時，若完成後可以[記錄時間]，也就是處理這張單子所花費的實際工時。
...
以上這些前提是執行力，在「MIS工作進度追蹤」平台的資料正確性及即時更新的執行力。要不然久了沒管理者看資料、MIS就開始沒更新、大家覺得那個平台的資訊荒廢已久與現實落差太大，所以又開始用Excel和Powerpoint石器時代的方式管理MIS，然後又走回每天處理**MIS鳥事**的惡性循環!