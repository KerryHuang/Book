...
1. 管理表單能不紙本就不紙本，盡量線上填單，記錄盡量變成資料庫內容，而不只是掃描紙本的PDF檔的管理。
...
雖然企業導入ISMS的目的不是為了要驗證，但不可否認的驗證ISO 27001已經是多數企業執行ISMS固定的年度最重要的活動。
...
- 部分紀錄或過程可能不屬於ISMS正式的四階表單，他屬於紀錄佐證或過程，也將其歸類在對應的條文或附錄。
- 紀錄分類舉例：
  ...
  - 例4：網路圖、拓樸圖是稽核必要的檢查項目，但不是正式的ISMS表單紀錄，所以我們將這些圖的檔案做好版本控制，放在[A.13 通訊安全的類別]。
