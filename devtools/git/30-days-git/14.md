---
kind: reprint
source: https://github.com/doggy8088/Learn-Git-in-30-days
author: 黃保翕(Will 保哥)
---
...
* 自動訂正打錯的參數
	...
	畢竟我們是用指令列工具在輸入，打錯字在所難免，例如你輸入 `git statsu` (打錯字了)，若啟用「自動訂正」選項，則 Git 工具會自動修正為 `git status` 並成功執行，讓你不用重打一次。以下是開啟自動訂正選項的指令：
