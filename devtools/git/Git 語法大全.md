...
- `git checkout -b <branch_name>`：創建並切換到新分支。
...
- `git remote add <name> <url>`：添加遠端儲存庫。
...
- `git tag <tag_name>`：創建輕量標籤。
- `git tag -a <tag_name> -m "message"`：創建帶訊息的註釋標籤。
...
- `git revert <commit>`：創建新提交，撤銷指定提交的更改。
- `git blame <file>`：顯示每行代碼的最後更改記錄（誰在何時修改）。
