...
到使用者資料夾`C:\Users\<user>\`中看是否有`.ssh`資料夾（`<user>`為你的Windows使用者名稱，本範例為`john`），如果有的話點進去看是否有下表中任一SSH keys檔案。下表的第一欄為加密方式，第二欄為公鑰檔名，第三欄為私鑰檔名。若SSH keys已存在則跳過「產生SSH keys」的步驟。

| 加密演算法                         | Public key       | Private key  |
| :------------------------------- | :--------------- | :----------- |
...
在C:\Users\使用者名稱\.ssh\資料夾中新建一個名稱為config的檔案，用記事本開啟
...
AddKeysToAgent yes
IdentityFile C:\Users\使用者名稱\.ssh\id_rsa

# 設定檔參數
# Host : Host可以看作是一個你要識別的模式，對識別的模式，進行設定對應的主機名稱和ssh檔案
# HostName : 要登入主機的主機名
# User : 登入名
