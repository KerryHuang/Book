...
![Blob 檔案架構](https://i.imgur.com/XodrYH2.png)

要使用 Azure Blob Storage 來儲存檔案，我們需要先了解「儲存體帳戶（Account）」、「容器（Container）」、「Blob」這三者的

<a name="建立儲存體帳戶"/>
## 建立儲存體帳戶

我們登入到 Azure 後台，點擊「所有服務」，搜尋「儲存體帳戶」就能找到。

![1](/Users/kerry/Library/CloudStorage/OneDrive-個人/公司/Soetek/My Briefcase/My Books/Markdown Cheatsheet 中文版/Azure Blob Storage/1.png)

1. 訂用帳戶：用來付錢的帳戶
2. 資源群組：每一個我們建立的 Azure 服務都會隸屬一個資源群組，假設我們正在開發一個專案，我們就可以用專案名稱來為資源群組命名，方便我們來管理專案所使用到的 Azure 服務。
3. 儲存體帳戶名稱
4. 區域：Azure 資料中心的位置
5. 效能：「標準」選項可以符合大多數的使用情境，如果想要獲得更高的存取效能，我們可以選用「進階」。
6. 備援：備份資料的方式，依據選擇的區域跟效能的不同，會有不同的選項，大致上分為「本地」跟「異地」兩種類型，有一些選項要額外付費，有一些不用，通常本地備援是不用另外收錢，我們可以參考官方文件 - [Azure 儲存體冗余](https://docs.microsoft.com/zh-tw/azure/storage/common/storage-redundancy)，裡面有更詳細的說明。
...
再回到儲存體帳戶底下，找到「`容器`」，在容器頁籤底下點擊「`＋容器`」來建立容器，我們有 2 個選項要填。

![](/Users/kerry/Library/CloudStorage/OneDrive-個人/公司/Soetek/My Briefcase/My Books/Markdown Cheatsheet 中文版/Azure Blob Storage/4.png)

1. 名稱
2. 公用存取層級：用來決定哪些資源可以「匿名讀取」，有`私人`、`Blob`、`容器` 3 個選項。

- 私人：Blob 及容器不允許被匿名讀取
- Blob：只有 Blob 可以被匿名讀取
- 容器：Blob 及容器都可以被匿名讀取

選項填完之後，我們就點擊「`建立`」將容器建起來。
...
最後是「`刪除 Blob`」，我建議使用 `DeleteIfExistsAsync()` 方法，如果單純使用 `DeleteAsync()` 方法的話，檔案不存在時，會有例外錯誤。
...
Azure Blob Storage 對於一些靜態檔案不多的網站來說，算是一個物美價廉的解決方案，標準經常性存取的儲存體，每月每 GB 空間的費用是 $0.024 鎂，折合台幣大約是 NT$0.664，一般的小型網站，靜態檔案要佔用到 1 GB 的空間其實很難