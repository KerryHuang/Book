---
kind: reprint
source: https://columns.chicken-house.net/2019/01/01/microservice12-mqrpc/
author: Andrew Wu
---

# [Part #4](https://columns.chicken-house.net/2019/01/01/microservice12-mqrpc/) 可靠的微服務通訊 - Message Queue Based RPC
...
1. 系列文章: [Thread Pool 實作](https://columns.chicken-house.net/categories/#系列文章: Thread Pool 實作)
2. 系列文章: [多執行緒的處理技巧](https://columns.chicken-house.net/categories/#系列文章: 多執行緒的處理技巧)
3. 參考文章: [在 .NET Console Application 中處理 Shutdown 事件](https://columns.chicken-house.net/2018/05/10/tips-handle-shutdown-event/)
4. 參考文章: [容器化的微服務開發 #2, IIS or Self Host ?](https://columns.chicken-house.net/2018/05/12/msa-labs2-selfhost/)
...
2. channel: virtual connection, [官方文件](https://www.rabbitmq.com/dotnet-api-guide.html#model-sharing) 建議別跨越 threads 共享 channel, 因為 channel 會強制 message 的處理是按照順序的, 多個 thread 的效能會受到影響。
3. consumer: 每個 consumer 會有專屬的 event handler (根據實驗, rabbitmq .net client 會替每個 consumer 準備 thread 來執行 event handler)。
...
![img](https://columns.chicken-house.net/wp-content/images/2019-01-01-microservice12-mqrpc/2019-01-01-18-21-34.png)
...
2. 接著連續送 10 個 message, 一口氣送完 (**不等**結果) 後，再一次等待 10 個結果回來
3. 接著送 “end” 的 message, 並且 **等待** 結果
...
2. 如果 client 已經離線, 這個訊息就可以丟掉了，整個 reply queue 的資源也可以放掉。
3. client 端等待回傳訊息應該只有一個，不需要多執行緒 (多個 rabbitmq connection / channel), 也不需要處理 graceful shutdown 的問題。
4. 如果單一個 client 發出多個 rpc call, 則可以共用一個 reply queue 降低系統負擔

接下來，一樣就看本事了。這裡考驗你幾個基本功夫:

1. 平行處理的技巧 (event, threads, wait / notify)
2. 對 RabbitMQ 的掌握能力
3. C# 的掌握能力 (async / await)
...
2. 先取得 `IServiceProvider`, 建立專用的 Scope 物件
3. 從 message headers 還原 `TrackContext`, 注入 Scope
4. 讓處理訊息的 delegate: `Process` 在這個 Scope 內運行。
5. 完成之後，回報 Ack, 告知 RabbitMQ 訊息處理完畢。 (若雙向的話還需要回傳 message)
...
2. 調整過程中你接觸的介面，是 cloud provider (如 Azure Portal, AWS Console … ) 提供的原生工具，還是自行開發的 management portal ?
3. 過程是全自動還是手動? 是否需要人力同時對 infrastructure / application 進行變更?
4. 能否在一定的時間內完成? (例如 1 min)
...
1. [Tips: 在 .NET Console Application 中處理 Shutdown 事件](https://columns.chicken-house.net/2018/05/10/tips-handle-shutdown-event/)
2. [容器化的微服務開發 #2, IIS or Self Host ?](https://columns.chicken-house.net/2018/05/12/msa-labs2-selfhost/)

Windows app 用 container 部署的相關主題:

1. [Azure Labs: Windows Container Swarm Mode](https://columns.chicken-house.net/2017/07/25/wc-swarm-labs/)
2. [Azure Labs: Mixed-OS Docker Swarm](https://columns.chicken-house.net/2017/07/25/wc-swarm-labs2/)
3. [LCOW Labs: Linux Container On Windows](https://columns.chicken-house.net/2017/10/04/lcow/)

Windows Container 踩雷紀錄:

1. [使用 LCOW 掛載 Volume 的效能陷阱](https://columns.chicken-house.net/2018/07/28/labs-lcow-volume/)
2. [掃雷回憶錄 - Windows Container Network & Docker Compose](https://columns.chicken-house.net/2017/01/15/docker-networks/)
...
<iframe src="https://www.facebook.com/plugins/video.php?href=https%3A%2F%2Fwww.facebook.com%2Fandrew.blog.0928%2Fvideos%2F355084961890233%2F&amp;show_text=0&amp;width=560" width="560" height="407" scrolling="no" frameborder="0" allowtransparency="true" allowfullscreen="true" style="box-sizing: border-box; color: rgb(51, 51, 51); font-family: Arial, &quot;Hiragino Sans GB&quot;, 冬青黑, &quot;Microsoft YaHei&quot;, 微软雅黑, SimSun, 宋体, Helvetica, Tahoma, &quot;Arial sans-serif&quot;; font-size: 16px; font-style: normal; font-variant-ligatures: normal; font-variant-caps: normal; font-weight: 400; letter-spacing: normal; orphans: 2; text-align: start; text-indent: 0px; text-transform: none; widows: 2; word-spacing: 0px; -webkit-text-stroke-width: 0px; white-space: normal; background-color: rgb(255, 255, 255); text-decoration-thickness: initial; text-decoration-style: initial; text-decoration-color: initial; border: none; overflow: hidden;"></iframe>
...
2. 安裝在 virtual machine 上
3. 透過 container 部署到單機 (docker, docker-compose)
4. 透過 container 部署到 orchestration 上 (kubernetes, docker swarm, azure / aws container services… etc)
